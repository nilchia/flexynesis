...
def build_network(
    expr_df, method="spearman", min_correlation=0.3, top_k=10, device=None
):
    """
    ...
    if method == "spearman":
        ...
        with tqdm(total=n_genes, desc="Converting to ranks") as pbar:
            for i in range(0, n_genes, batch_size):
                ...
                batch = data[i:end_i]
                ranks[i:end_i] = torch.argsort(
                    torch.argsort(batch, dim=1), dim=1
                ).float()
                pbar.update(end_i - i)
        data = ranks
    elif method != "pearson":
    ...
    print(
        f"Computing correlations and building network (min |r| = {min_correlation}, top {top_k} per gene)..."
    )
            ...
            for local_idx, global_idx in enumerate(range(i, end_i)):
                ...
                if mask.sum() > top_k:
                    ...
                    top_k_values, top_k_indices = torch.topk(
                        abs_corr, min(top_k, len(abs_corr))
                    )
...
def generate_coexpression_network(
    ...
    print(
        f"  All unique genes: {len(set(network_df['GeneA']) | set(network_df['GeneB'])):,}"
    )
    print(
        f"  Score range: [{network_df['Score'].min():.4f}, {network_df['Score'].max():.4f}]"
    )
